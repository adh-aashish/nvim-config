...
command! -nargs=0 CompileAndRun call TermWrapper(printf('g++ -std=c++20 %s && ./a.out && rm ./a.out' , expand('%')))
command! -nargs=0 SfmlRun call TermWrapper(printf('g++ -std=c++11 -c %s && g++ main.o -o sfml-app -lsfml-graphics -lsfml-window -lsfml-system  && ./sfml-app' , expand('%')))
command! -nargs=1 CompileAndRunWithFile call TermWrapper(printf('g++ -std=c++11 %s && ./a.out < %s', expand('%'), <args>))
autocmd FileType cpp nnoremap <leader>fw :CompileAndRun<CR>
autocmd FileType cpp nnoremap <leader>sf :SfmlRun<CR>
